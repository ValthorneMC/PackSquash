...
impl PackSquasher {
		...
		PackSquasherError: From<<O as TryInto<ProcessedSquashOptions>>::Error>
	{
		...
		// Directories declared as pack.mcmeta "overlays", if any. Pack files found under
		// them are treated as if they were directly under the pack root for the purposes
		// of asset type recognition (see the overlay handling below), so that packs (or
		// pack fragments generated by third-party plugins that reuse this mechanism) that
		// rely on overlays are optimized just as well as files under plain assets/data
		...
		if read_pack_meta {
			runtime.block_on(async {
				...
				overlay_directories = pack_meta.overlay_directories().to_vec();
			...
			// None of the other options that need pack.mcmeta are enabled, but we still want
			// overlay directories to be detected out of the box. Unlike the block above, this
			// is best-effort: errors reading or parsing pack.mcmeta are silently ignored here,
			// since nothing else in this branch actually requires a valid pack.mcmeta to exist
			if let Ok(pack_meta) =
				runtime.block_on(PackMeta::new(&vfs, &options_holder.options.pack_directory))
			{
				overlay_directories = pack_meta.overlay_directories().to_vec();
			}
		}

		let overlay_directories = Arc::new(overlay_directories);
		let vfs = Arc::new(vfs);
		...
		runtime.block_on(async {
			...
			for pack_file_data in pack_file_iter {
				...
				let overlay_directories = Arc::clone(&overlay_directories);
				...
				pack_file_tasks.push(runtime.spawn(async move {
					...
					let asset_type_matches = {
						let mut asset_type_matches =
							asset_type_matcher.matches_for(&pack_file_data.relative_path);

						// If the path wasn't recognized as-is, but it lives under a
						// pack.mcmeta-declared overlay directory, retry the match after
						// stripping that directory component. This lets pack files placed
						// in overlays (be it Mojang's own multi-version pack mechanism, or
						// plugins that generate their own overlay-like fragments) be
						// recognized and optimized just like their assets/data counterparts,
						// instead of being skipped as unrecognized custom files
						if asset_type_matches.is_empty()
							&& let Some(overlay_relative_path) = overlay_directories
								.iter()
								.find_map(|dir| pack_file_data.relative_path.strip_overlay_prefix(dir))
						{
							asset_type_matches = asset_type_matcher.matches_for(&overlay_relative_path);
						}
